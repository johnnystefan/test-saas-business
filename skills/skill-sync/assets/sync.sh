...
get_agents_path() {
    ...
    case "$scope" in
        ...
        booking-service)   echo "$REPO_ROOT/apps/booking-service/AGENTS.md" ;;
        finance-service)   echo "$REPO_ROOT/apps/finance-service/AGENTS.md" ;;
        libs)              echo "$REPO_ROOT/libs/AGENTS.md" ;;
        *)                 echo "" ;;
    esac
}
...
echo -e "${BLUE}Skill Sync - SaaS Business Platform${NC}"
echo -e "${BLUE}Updating AGENTS.md Auto-invoke sections${NC}"
echo "========================================================"
echo ""
...
done < <(find "$SKILLS_DIR" -mindepth 2 -name SKILL.md -print | sort)
