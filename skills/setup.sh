...
setup_opencode() {
    ...
    # Remove existing symlink or empty dir — backup if it has content
    if [ -L "$target" ]; then
        rm "$target"
    elif [ -d "$target" ]; then
        if [ "$(ls -A "$target" 2>/dev/null)" ]; then
            local backup="$REPO_ROOT/.opencode/skills.backup.$(date +%s)"
            mv "$target" "$backup"
            echo -e "${YELLOW}  ! Existing skills/ backed up to: $backup${NC}"
        else
            rmdir "$target"
        fi
    fi

    # Use absolute path so symlink works from any working directory
    ln -s "$SKILLS_SOURCE" "$target"
    echo -e "${GREEN}  ✓ .opencode/skills -> $SKILLS_SOURCE${NC}"
}
...
show_menu() {
    ...
    local selected=(false false false false true)  # OpenCode selected by default
...
[ "$SETUP_OPENCODE" = true ] && echo "  • OpenCode:       .opencode/skills/"
echo ""
